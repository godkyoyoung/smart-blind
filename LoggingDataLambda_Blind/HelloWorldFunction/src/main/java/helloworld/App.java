...
public class App implements RequestHandler<Document, String> {
    ...
    private String persistData(Document document) throws ConditionalCheckFailedException {
        ...
        // Sunshine과 Condition 값이 이전상태와 동일한 경우 테이블에 저장하지 않고 종료
        if (document.current.state.reported.SunShine.equals(
                document.previous.state.reported.SunShine) &&
                document.current.state.reported.Condition.equals(
                        document.previous.state.reported.Condition)) {
            return null;
        }
        ...
        return this.dynamoDb.getTable(DYNAMODB_TABLE_NAME)
                .putItem(new PutItemSpec().withItem(new Item()
                        ...
                        .withString("SunShine", document.current.state.reported.SunShine)
                        .withString("Condition", document.current.state.reported.Condition)
